...
class TestCreateCourier:
    @allure.title('Успешное создание курьера статус код 201')
    ...
    @allure.title('Успешное создание курьера, вернулось корректное тело запроса')
    ...
    def test_successful_courier_created_with_correct_response_body(self):
        ...
        assert response.text == ResponseBody.create_courier_success
    ...
    def test_successful_courier_created_only_with_required_fields(self):
        ...
        assert response.status_code == StatusCode.create_status_code and response.text == ResponseBody.create_courier_success
    ...
    def test_created_courier_without_all_required_fields(self, courier_data):
        ...
        assert response.status_code == StatusCode.bad_request_status_code and ResponseBody.create_courier_without_data == r["message"]
    ...
    def test_created_courier_with_repeated_login(self, register_new_courier_and_return_login_password):
        ...
        assert response.status_code == StatusCode.conflict_status_code and ResponseBody.create_courier_with_repeated_data == r["message"]
